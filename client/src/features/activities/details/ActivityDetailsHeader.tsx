...
export default function ActivityDetailsHeader({activity}: Props) {
    ...
    return (
        ...
        {activity.isCancelled && (
            <Chip
                sx={{ position: 'absolute', left: 40, top: 20, zIndex: 1000, borderRadius: 3 }}
                color="error"
                label="Cancelled"
            />
        )}
        ...
        <Box sx={{
            ...
            <Box>
                ...
                <Typography variant="subtitle2">
                    Hosted by <Link to={`/profiles/${activity.hostId}`} style={{ color: 'white', fontWeight: 'bold' }}>{activity.hostDisplayName}
                    </Link>
                </Typography>
            </Box>
